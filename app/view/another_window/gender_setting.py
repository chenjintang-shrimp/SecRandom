...
class GenderSettingWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # 创建说明标签
        self.description_label = BodyLabel(get_content_name_async("gender_setting", "description"))
    ...
    def __create_gender_input_area(self):
        ...
        # 创建输入区域标题
        input_title = SubtitleLabel(get_content_name_async("gender_setting", "input_title"))
        input_layout.addWidget(input_title)
        
        # 创建文本编辑框
        self.text_edit = PlainTextEdit()
        self.text_edit.setPlaceholderText(get_content_name_async("gender_setting", "input_placeholder"))
    ...
    def __create_button_area(self):
        ...
        # 保存按钮
        self.save_button = PrimaryPushButton(get_content_name_async("gender_setting", "save_button"))
        self.save_button.setIcon(FluentIcon.SAVE)
        button_layout.addWidget(self.save_button)
        
        # 取消按钮
        self.cancel_button = PushButton(get_content_name_async("gender_setting", "cancel_button"))
    ...
    def __on_text_changed(self):
        ...
        current_genders = [gender.strip() for gender in current_text.split('\n') if gender.strip()]
        
        # 检查哪些初始性别被删除了
        deleted_genders = [gender for gender in self.initial_genders if gender not in current_genders]
        ...
        if deleted_genders:
            for gender in deleted_genders:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("gender_setting", "gender_deleted_title"),
                    content=get_content_name_async("gender_setting", "gender_deleted_message").format(gender=gender),
                    duration=3000
                )
        ...
        try:
            ...
            if not genders_text:
                ...
                config = NotificationConfig(
                    ...
                    content=get_content_name_async("gender_setting", "no_genders_error"),
                    duration=3000
                )
            ...
            # 分割性别
            genders = [gender.strip() for gender in genders_text.split('\n') if gender.strip()]
            ...
            if invalid_genders:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("gender_setting", "error_title"),
                    content=get_content_name_async("gender_setting", "invalid_genders_error").format(
                        genders=", ".join(invalid_genders)
                    ),
                    duration=5000
                )
                show_notification(NotificationType.ERROR, config, parent=self)
            ...
            config = NotificationConfig(
                ...
                content=get_content_name_async("gender_setting", "success_message").format(
                    count=len(genders)
                ),
                duration=3000
            )
    ...
    def closeEvent(self, event):
        ...
        if not self.saved:
            ...
            dialog.yesButton.setText(get_content_name_async("gender_setting", "discard_button"))
            dialog.cancelButton.setText(get_content_name_async("gender_setting", "continue_editing_button"))
