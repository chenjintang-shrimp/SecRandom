...
class ImportStudentNameWindow(QWidget):
    ...
    def __init_ui(self):
        ...
        # 创建标题
        self.title_label = TitleLabel(get_content_name_async("import_student_name", "title"))
    ...
    def __create_file_selection_area(self):
        ...
        # 创建文件选择区域标题
        file_title = SubtitleLabel(get_content_name_async("import_student_name", "file_selection_title"))
        ...
        # 文件路径标签
        self.file_path_label = BodyLabel(get_content_name_async("import_student_name", "no_file_selected"))
        self.file_path_label.setWordWrap(True)
        file_row.addWidget(self.file_path_label, 1)
        
        # 选择文件按钮
        self.select_file_btn = PrimaryPushButton(get_content_name_async("import_student_name", "select_file"))
        ...
        # 支持格式提示
        format_hint = CaptionLabel(get_content_name_async("import_student_name", "supported_formats"))
    ...
    def __create_column_mapping_area(self):
        ...
        # 创建列映射区域标题
        mapping_title = SubtitleLabel(get_content_name_async("import_student_name", "column_mapping_title"))
        mapping_layout.addWidget(mapping_title)
        
        # 创建列映射说明
        mapping_desc = BodyLabel(get_content_name_async("import_student_name", "column_mapping_description"))
        ...
        # 学号列选择（必选项，第一个）
        id_row = QHBoxLayout()
        id_label = BodyLabel(get_content_name_async("import_student_name", "column_mapping_id_column"))
        ...
        name_label = BodyLabel(get_content_name_async("import_student_name", "column_mapping_name_column"))
        self.name_column_combo = ComboBox()
        self.name_column_combo.currentIndexChanged.connect(self.__on_column_mapping_changed)
        ...
        self.gender_column_combo.currentIndexChanged.connect(self.__on_column_mapping_changed)
        ...
        group_label = BodyLabel(get_content_name_async("import_student_name", "column_mapping_group_column"))
        self.group_column_combo = ComboBox()
        self.group_column_combo.currentIndexChanged.connect(self.__on_column_mapping_changed)
    ...
    def __create_preview_area(self):
        ...
        # 创建预览区域标题
        preview_title = SubtitleLabel(get_content_name_async("import_student_name", "data_preview_title"))
    ...
    def __create_button_area(self):
        ...
        # 导入按钮
        self.import_btn = PrimaryPushButton(get_content_name_async("import_student_name", "buttons_import"))
    ...
    def __update_ui_state(self):
        ...
        # 检查是否选择了"无"选项
        if id_column == get_content_name_async("import_student_name", "column_mapping_none"):
            id_column = ""
        if name_column == get_content_name_async("import_student_name", "column_mapping_none"):
            name_column = ""
        ...
        if file_path:
            try:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("import_student_name", "file_loaded_notification_title"),
                    content=get_content_name_async("import_student_name", "file_loaded_notification_content"),
                    duration=3000
                )
    ...
    def __update_preview(self):
        ...
        if name_column == get_content_name_async("import_student_name", "column_mapping_none"):
            name_column = None
        if gender_column == get_content_name_async("import_student_name", "column_mapping_none"):
            gender_column = None
        if group_column == get_content_name_async("import_student_name", "column_mapping_none"):
            group_column = None
        ...
        if id_column:
            ...
            preview_headers.append(get_content_name_async("import_student_name", "student_id"))
        
        if name_column:
            preview_columns.append(name_column)
            preview_headers.append(get_content_name_async("import_student_name", "name"))
            
        if gender_column:
            preview_columns.append(gender_column)
            preview_headers.append(get_content_name_async("import_student_name", "gender"))
            
        if group_column:
            preview_columns.append(group_column)
            preview_headers.append(get_content_name_async("import_student_name", "group"))
    ...
    def __import_data(self):
        ...
        try:
            ...
            # 验证必选项：学号和姓名列都必须选择
            if not id_column:
                raise ValueError(get_content_name_async("import_student_name", "no_id_column"))
            ...
            # 显示成功消息
            config = NotificationConfig(
                title=get_content_name_async("import_student_name", "import_success_notification_title"),
                content=get_content_name_async("import_student_name", "import_success_notification_content_template").format(
                    count=len(student_data), class_name=class_name
                ),
                duration=3000
            )
            show_notification(NotificationType.SUCCESS, config, parent=self)
            ...
            # 显示错误消息
            config = NotificationConfig(
                title=get_content_name_async("import_student_name", "import_failed_notification_title"),
    ...
    def __save_student_data(self, class_name: str, student_data: List[Dict[str, Any]]):
        ...
        if existing_data:
            ...
            dialog = MessageBox(
                ...
                get_content_name_async("import_student_name", "existing_data_prompt").format(
                    class_name=class_name, count=len(existing_data)
                ),
                self
            )
            
            dialog.yesButton.setText(get_content_name_async("import_student_name", "existing_data_option_overwrite"))
            dialog.cancelButton.setText(get_content_name_async("import_student_name", "existing_data_option_cancel"))
            ...
            if dialog.exec():
                ...
                for student in student_data:
                    all_students[student['name']] = {
                        "id": int(student['id']) if str(student['id']).isdigit() else student['id'],
        ...
        if action == "overwrite":
            logger.info(f"已覆盖班级 '{class_name}' 的数据，共 {len(all_students)} 名学生")
        else:
