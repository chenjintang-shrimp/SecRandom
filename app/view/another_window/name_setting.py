...
class NameSettingWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # 创建说明标签
        self.description_label = BodyLabel(get_content_name_async("name_setting", "description"))
    ...
    def __create_name_input_area(self):
        ...
        self.text_edit.setPlaceholderText(get_content_name_async("name_setting", "input_placeholder"))
    ...
    def __create_button_area(self):
        ...
        # 保存按钮
        self.save_button = PrimaryPushButton(get_content_name_async("name_setting", "save_button"))
        self.save_button.setIcon(FluentIcon.SAVE)
        button_layout.addWidget(self.save_button)
        
        # 取消按钮
        self.cancel_button = PushButton(get_content_name_async("name_setting", "cancel_button"))
    ...
    def __on_text_changed(self):
        ...
        current_names = [name.strip() for name in current_text.split('\n') if name.strip()]
        
        # 检查哪些初始姓名被删除了
        deleted_names = [name for name in self.initial_names if name not in current_names]
        ...
        if deleted_names:
            for name in deleted_names:
                ...
                config = NotificationConfig(
                    ...
                    content=get_content_name_async("name_setting", "name_deleted_message").format(name=name),
                    duration=3000
                )
    ...
    def __save_names(self):
        ...
        try:
            ...
            if invalid_names:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("name_setting", "error_title"),
                    content=get_content_name_async("name_setting", "invalid_names_error").format(
            ...
            if set(names) == set(existing_data.keys()):
                ...
                config = NotificationConfig(
                    ...
                    content=get_content_name_async("name_setting", "no_new_names_message"),
                    duration=3000
                )
                show_notification(NotificationType.INFO, config, parent=self)
                return
            ...
            config = NotificationConfig(
                ...
                content=get_content_name_async("name_setting", "success_message").format(
                    count=len(names)
                ),
                duration=3000
            )
    ...
    def closeEvent(self, event):
        ...
        if not self.saved:
            ...
            dialog.yesButton.setText(get_content_name_async("name_setting", "discard_button"))
            dialog.cancelButton.setText(get_content_name_async("name_setting", "continue_editing_button"))
