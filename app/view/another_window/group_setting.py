...
class GroupSettingWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # 创建说明标签
        self.description_label = BodyLabel(get_content_name_async("group_setting", "description"))
    ...
    def __create_group_input_area(self):
        ...
        # 创建输入区域标题
        input_title = SubtitleLabel(get_content_name_async("group_setting", "input_title"))
        input_layout.addWidget(input_title)
        
        # 创建文本编辑框
        self.text_edit = PlainTextEdit()
        self.text_edit.setPlaceholderText(get_content_name_async("group_setting", "input_placeholder"))
    ...
    def __create_button_area(self):
        ...
        # 保存按钮
        self.save_button = PrimaryPushButton(get_content_name_async("group_setting", "save_button"))
        self.save_button.setIcon(FluentIcon.SAVE)
        button_layout.addWidget(self.save_button)
        
        # 取消按钮
        self.cancel_button = PushButton(get_content_name_async("group_setting", "cancel_button"))
    ...
    def __on_text_changed(self):
        ...
        current_groups = [group.strip() for group in current_text.split('\n') if group.strip()]
        
        # 检查哪些初始小组被删除了
        deleted_groups = [group for group in self.initial_groups if group not in current_groups]
        ...
        if deleted_groups:
            for group in deleted_groups:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("group_setting", "group_deleted_title"),
                    content=get_content_name_async("group_setting", "group_deleted_message").format(group=group),
                    duration=3000
                )
    ...
    def __save_groups(self):
        ...
        try:
            ...
            if invalid_groups:
                ...
                config = NotificationConfig(
                    title=get_content_name_async("group_setting", "error_title"),
                    content=get_content_name_async("group_setting", "invalid_groups_error").format(
            ...
            if set(groups) == set(existing_groups):
                ...
                config = NotificationConfig(
                    ...
                    content=get_content_name_async("group_setting", "no_new_groups_message"),
                    duration=3000
                )
                show_notification(NotificationType.INFO, config, parent=self)
                return
            ...
            config = NotificationConfig(
                ...
                content=get_content_name_async("group_setting", "success_message").format(
                    count=len(groups)
                ),
                duration=3000
            )
    ...
    def closeEvent(self, event):
        ...
        if not self.saved:
            ...
            dialog.yesButton.setText(get_content_name_async("group_setting", "discard_button"))
            dialog.cancelButton.setText(get_content_name_async("group_setting", "continue_editing_button"))
