...
class SetPasswordWindow(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle(get_content_name_async("basic_safety_settings", "set_password"))
        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(20, 20, 20, 20)
        self.main_layout.setSpacing(15)

        self.title_label = TitleLabel(get_content_name_async("basic_safety_settings", "set_password"))
        self.main_layout.addWidget(self.title_label)

        self.description_label = BodyLabel(get_content_description_async("basic_safety_settings", "set_password"))
        ...
        rules_title = SubtitleLabel(get_content_name_async("basic_safety_settings", "password_rules"))
        rules_desc = BodyLabel(get_content_description_async("basic_safety_settings", "password_rules"))
        rules_desc.setWordWrap(True)
        input_layout.addWidget(rules_title)
        input_layout.addWidget(rules_desc)

        self.current_label = BodyLabel(get_content_name_async("basic_safety_settings", "current_password"))
        self.current_edit = PasswordLineEdit()
        self.new_label = BodyLabel(get_content_name_async("basic_safety_settings", "new_password"))
        self.new_edit = PasswordLineEdit()
        self.confirm_label = BodyLabel(get_content_name_async("basic_safety_settings", "confirm_password"))
        self.confirm_edit = PasswordLineEdit()

        self.strength_label = BodyLabel(get_content_name_async("basic_safety_settings", "password_strength_title"))
        ...
        self.remove_button = PushButton(get_content_name_async("basic_safety_settings", "remove_password"))
    ...
    def __update_strength(self):
        ...
        if any(c in "!@#$%^&*()-_=+[]{};:'\",.<>/?|`~" for c in t):
            score += 1
        if score <= 1:
            self.strength_label.setText(get_content_name_async("basic_safety_settings", "strength_weak"))
            self.strength_label.setStyleSheet("color: #d9534f;")
        elif score <= 3:
            self.strength_label.setText(get_content_name_async("basic_safety_settings", "strength_medium"))
            self.strength_label.setStyleSheet("color: #f0ad4e;")
        else:
            self.strength_label.setText(get_content_name_async("basic_safety_settings", "strength_strong"))
    ...
    def __remove_password(self):
        try:
            ...
            dialog = MessageBox(
                ...
                get_content_name_async("basic_safety_settings","remove_password_confirm_content"),
                self,
            )
            dialog.yesButton.setText(get_content_name_async("basic_safety_settings","dialog_yes_text"))
            dialog.cancelButton.setText(get_content_name_async("basic_safety_settings","dialog_cancel_text"))
            if dialog.exec():
                clear_password()
