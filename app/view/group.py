...
class groupplayer(QWidget):
    ...
    def start_draw(self):
        ...
        try:
            with open('app/Settings/Settings.json', 'r', encoding='utf-8') as f:
                ...
                group_player_animation_mode = settings['group_player']['animation_mode']
                global_draw_mode = settings['global']['draw_mode']
                if group_player_animation_mode == 0:
                    global_animation_mode = settings['global']['animation_mode']
        except Exception as e:
            # group_player_draw_mode = 1
            group_player_animation_mode = 1
        ...
        # 根据动画模式执行不同逻辑
        if group_player_animation_mode == 0 and global_animation_mode == 0:  # 手动停止动画
        ...
        elif group_player_animation_mode == 0 and global_animation_mode == 1:  # 自动播放完整动画
            self._play_full_animation()
            
        elif group_player_animation_mode == 0 and global_animation_mode == 2:  # 直接显示结果
            self._show_result_directly()
            
        # 跟随单抽动画设置
        elif group_player_animation_mode == 1:  # 手动停止动画
            self.start_button.setText("停止")
            self.is_animating = True
            self.animation_timer = QTimer()
            self.animation_timer.timeout.connect(self._show_random_student)
        ...
        elif group_player_animation_mode == 2:  # 自动播放完整动画
            self._play_full_animation()
            
        elif group_player_animation_mode == 3:  # 直接显示结果
            self._show_result_directly()
                ...
                with open(student_file, 'r', encoding='utf-8') as f:
                    ...
                    if students:
                        ...
                        for num, name in selected_students:
                            # 整合学号格式和姓名处理逻辑
                            student_id_format = single_player_student_id if single_player_student_id != 0 else global_student_id
                            student_name_format = single_player_student_name if single_player_student_name != 0 else global_student_name
    ...
    def _stop_animation(self):
        ...
        try:
            with open('app/Settings/Settings.json', 'r', encoding='utf-8') as f:
                settings = json.load(f)
                voice_enabled = settings['group_player']['voice_enabled']
                if voice_enabled == 0:
                        ...
                        for num, selected in selected_students:
                            ...
                            if os.path.exists(group_file):
                                with open(group_file, 'r', encoding='utf-8') as f:
                                    group_students = [line.strip().replace(' ', '') for line in f if line.strip() and not line.strip().startswith('【') and not line.strip().endswith('】')]
                            ...
                            if selected not in group_students:
                                if history_enabled:
                                    ...
                                    if selected not in history_data["group"]:
                                        history_data["group"][selected] = {
                                        ...
                                        history_data["group"][selected]["total_number_of_times"] += 1
                                        history_data["group"][selected]["time"].append({
                                            "draw_method": {
                                                self.draw_mode: datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                            }
                                        })
    ...
    def refresh_class_group_list(self):
        """刷新班级下拉框选项"""
    ...
    def initUI(self):
        ...
        self.refresh_button.clicked.connect(self.refresh_class_group_list)
        ...
        class_name = self.class_combo.currentText()
        try:
            class_file = f"app/resource/group/{class_name}_group.ini"
            if os.path.exists(class_file):
                with open(class_file, 'r', encoding='utf-8') as f:
                    classes = [line.strip() for line in f.read().split('\n') if line.strip()]
                    if classes:
                        self.group_combo.addItems(classes)
                    else:
                        logger.error(f"你暂未添加小组")
                        self.group_combo.addItem("你暂未添加小组")
            else:
                logger.error(f"你暂未添加小组")
                self.group_combo.addItem("你暂未添加小组")
