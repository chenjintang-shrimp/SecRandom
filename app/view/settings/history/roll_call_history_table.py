...
class roll_call_history_table(GroupHeaderCardWidget):
    ...
    def fill_all_students_data(self, class_name: str):
        ...
        for row, student in enumerate(students):
            # 学号
            id_item = QTableWidgetItem(str(student.get('student_id', student.get('id', row + 1))))
            ...
            name_item.setFlags(name_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 姓名不可编辑
            name_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.table.setItem(row, 1, name_item)
            
            # 性别
            gender_item = QTableWidgetItem(student.get('gender', ''))
            gender_item.setFlags(gender_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 性别不可编辑
            ...
            group_item.setFlags(group_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 小组不可编辑
            ...
            total_count_item.setFlags(total_count_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 总次数不可编辑
    ...
    def fill_time_mode_data(self, class_name: str):
        ...
        for row, session in enumerate(sessions):
            ...
            time_item.setFlags(time_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 时间不可编辑
    ...
    def fill_individual_mode_data(self, class_name: str):
        ...
        for row, stat in enumerate(stats):
            ...
            mode_item.setFlags(mode_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 模式不可编辑
            ...
            people_count_item.setFlags(people_count_item.flags() & ~Qt.ItemFlag.ItemIsEditable)  # 人数不可编辑
