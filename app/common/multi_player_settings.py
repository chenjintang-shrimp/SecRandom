...
class multi_player_SettinsCard(GroupHeaderCardWidget):
    def __init__(self, parent=None):
        ...
        self.multi_player_Draw_comboBox = ComboBox()
        ...
        # 抽取模式下拉框
        self.multi_player_Draw_comboBox.setFixedWidth(320) # 设置下拉框宽度为320像素
        self.multi_player_Draw_comboBox.addItems(["跟随全局设置", "重复抽取", "不重复抽取(直到软件重启)", "不重复抽取(直到抽完全部人)"])
        self.multi_player_Draw_comboBox.currentIndexChanged.connect(self.save_settings)
        ...
        self.multi_player_student_id_comboBox.addItems(["跟随全局设置", "⌈01⌋", "⌈ 1 ⌋", "⌈  1⌋"])
        ...
        self.multi_player_student_name_comboBox.addItems(["跟随全局设置", "⌈张  三⌋", "⌈张三  ⌋", "⌈  张三⌋"])
        ...
        self.multi_player_student_quantity_comboBox.addItems(["跟随全局设置", "显示", "不显示"])
        ...
        self.multi_player_class_quantity_comboBox.addItems(["跟随全局设置", "显示", "不显示"])
        self.multi_player_class_quantity_comboBox.currentIndexChanged.connect(self.save_settings)

        # 添加组件到分组中
        self.addGroup(FIF.SYNC, "抽取模式", "设置抽取模式", self.multi_player_Draw_comboBox)
        ...
        self.addGroup(FIF.EDUCATION, "便捷修改班级", "设置便捷修改班级功能是否显示", self.multi_player_class_quantity_comboBox)
        ...
        try:
            if os.path.exists(self.settings_file):
                ...
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    # 优先使用保存的文字选项
                    draw_mode_text = multi_player_settings.get("draw_mode_text", self.multi_player_Draw_comboBox.itemText(self.default_settings["draw_mode"]))
                    draw_mode = self.multi_player_Draw_comboBox.findText(draw_mode_text)
                    if draw_mode == -1:
                        # 如果文字选项无效，则使用索引值
                        logger.warning(f"无效的抽取模式文本: {draw_mode_text}")
                        draw_mode = multi_player_settings.get("draw_mode", self.default_settings["draw_mode"])
                        if draw_mode < 0 or draw_mode >= self.multi_player_Draw_comboBox.count():
                            # 如果索引值无效，则使用默认值  
                    ...
                    student_quantity_text = multi_player_settings.get("student_quantity_text", self.multi_player_student_quantity_comboBox.itemText(self.default_settings["student_quantity_enabled"]))
                    ...
                    if student_quantity == -1:
                        ...
                        student_quantity = multi_player_settings.get("student_quantity", self.default_settings["student_quantity_enabled"])
                    ...
                    class_quantity_text = multi_player_settings.get("class_quantity_text", self.multi_player_class_quantity_comboBox.itemText(self.default_settings["class_quantity_enabled"]))
                    ...
                    if class_quantity == -1:
                        ...
                        class_quantity = multi_player_settings.get("class_quantity", self.default_settings["class_quantity_enabled"])
                    ...
                    self.multi_player_Draw_comboBox.setCurrentIndex(draw_mode)
                ...
                self.multi_player_Draw_comboBox.setCurrentIndex(self.default_settings["draw_mode"])
                ...
                self.multi_player_student_quantity_comboBox.setCurrentIndex(self.default_settings["student_quantity_enabled"])
                self.multi_player_class_quantity_comboBox.setCurrentIndex(self.default_settings["class_quantity_enabled"])
                self.save_settings()
    ...
    def save_settings(self):
        ...
        # 保存文字选项
        multi_player_settings["draw_mode_text"] = self.multi_player_Draw_comboBox.currentText()
        ...
        # 同时保存索引值
        multi_player_settings["draw_mode"] = self.multi_player_Draw_comboBox.currentIndex()
