...
class ContributorDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        for contributor in contributors:
            ...
            text_rect = fm.boundingRect(
                QRect(0, 0, 500, 0), Qt.TextFlag.TextWordWrap, role_text
            )
    ...
    def update_theme_style(self):
        ...
        # 主题样式更新
        colors = (
            {"text": "#F5F5F5", "bg": "#111116", "title_bg": "#2D2D2D"}
            if is_dark
            else {"text": "#111116", "bg": "#F5F5F5", "title_bg": "#E0E0E0"}
        )
        ...
        if os.name == "nt":
            try:
                ...
                # 转换为ARGB格式（添加不透明通道）
                rgb_color = (
                    int(f"FF{bg_color}", 16)
                    if len(bg_color) == 6
                    else int(bg_color, 16)
                )
