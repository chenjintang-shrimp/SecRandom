...
def record_drawn_student(class_name: str, gender: str, group: str, student_name):
    ...
    file_path = get_resources_path("TEMP", f"draw_until_{class_name}_{gender}_{group}.json")
...
def remove_record(class_name: str, gender: str, group: str, _prefix: str = "0"):
    ...
    if prefix == "all":
        ...
        search_pattern = os.path.join(
            "app", "resources", "TEMP", 
            f"draw_*_{class_name}_{gender}_{group}.json"
        )
        ...
        # 只删除特定前缀的文件
        file_path = get_resources_path("TEMP", f"draw_{prefix}_{class_name}_{gender}_{group}.json")
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                file_name = os.path.basename(os.path.dirname(file_path))
...
def reset_drawn_record(self, class_name: str, gender: str, group: str):
    ...
    if clear_record in [0, 1]: # 重启后清除、直至全部抽取完
        ...
        logger.info(f"当前处于重复抽取状态，无需清除{class_name}_{gender}_{group}已抽取记录")

# ======= 计算剩余人数 =======
def calculate_remaining_count(half_repeat: int, class_name: str, gender_filter: str, group_filter: str, total_count: int):
    ...
    if half_repeat > 0:  # 只有当设置值大于0时才计算排除后的剩余人数
        ...
        for student in student_list:
            # 从学生字典中提取姓名
            student_name = student['name'] if isinstance(student, dict) and 'name' in student else student
            # 如果学生已被抽取次数达到或超过设置值，则计入排除数量
            if student_name in drawn_counts and drawn_counts[student_name] >= half_repeat:
                excluded_count += 1
