...
def calculate_weight(students_data: list, class_name: str) -> list:
    ...
    settings = {
        ...
        'fair_draw_group_enabled': readme_settings_async("advanced_settings", "fair_draw_group"),
        'fair_draw_gender_enabled': readme_settings_async("advanced_settings", "fair_draw_gender"),
        'fair_draw_time_enabled': readme_settings_async("advanced_settings", "fair_draw_time"),
        ...
        if isinstance(students_history, dict):
            for student_name, student_info in students_history.items():
                if student_name in weight_data and isinstance(student_info, dict):
                    # 更新总计数和最后抽取时间
                    weight_data[student_name]['total_count'] = student_info.get("total_count", 0)
                    weight_data[student_name]['rounds_missed'] = student_info.get("rounds_missed", 0)
                    ...
                    if isinstance(history, list):
                        for record in history:
                            if isinstance(record, dict):
                                ...
                                if draw_group and draw_group != get_content_combo_name_async("roll_call", "range_combobox")[0]:
                                    weight_data[student_name]['group_count'] += 1
                                    
                                # 更新性别计数
                                draw_gender = record.get("draw_gender", "")
                                if draw_gender and draw_gender != get_content_combo_name_async("roll_call", "gender_combobox")[0]:
    ...
    for student in students_data:
        ...
        if settings['fair_draw_enabled']:
            ...
            frequency_factor = 1.0 / math.sqrt(student_weight_data['total_count'] * 2 + 1)
        ...
        if settings['fair_draw_group_enabled']:
            ...
            if len(valid_groups) > 3:  # 有效小组数量达标
                ...
                if max_group_count == 0:
                    group_balance = 0.2  # 给所有学生一个小组平衡的基础权重提升，保持公平
                elif student_weight_data['group_count'] == 0:
                    group_balance = 0.5  # 给从未在小组中被选中的学生一个基础权重提升
                else:
                    group_balance = 0.8 * (1.0 - (student_weight_data['group_count'] / max_group_count))
        else:
            group_balance = 0.0
        ...
        if settings['fair_draw_gender_enabled']:
            ...
            if len(valid_gender) > 3:  # 有效性别数量达标
                ...
                if max_gender_count == 0:
                    gender_balance = 0.2  # 给所有学生一个性别平衡的基础权重提升，保持公平
                elif student_weight_data['gender_count'] == 0:
                    gender_balance = 0.5  # 给从未在性别平衡中被选中的学生一个基础权重提升
                else:
                    gender_balance = 0.8 * (1.0 - (student_weight_data['gender_count'] / max_gender_count))
        else:
            gender_balance = 0.0
            
        # 计算时间因子
        if settings['fair_draw_time_enabled'] and student_weight_data['last_drawn_time']:
            try:
                from datetime import datetime
                last_time = datetime.fromisoformat(student_weight_data['last_drawn_time'])
        ...
        # 确保权重在最小和最大值之间
        total_weight = max(settings['min_weight'], min(settings['max_weight'], total_weight))
...
def get_all_names(history_type: str, list_name: str) -> list:
    ...
    try:
        if history_type == "roll_call":
            ...
            return [item["name"] for item in list_name_data if "name" in item and item["name"]]
        else:
