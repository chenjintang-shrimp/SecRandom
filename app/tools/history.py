...
def get_individual_statistics(class_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        individual_stats.append({
            ...
            "group": ", ".join([f"{group}:{count}" for group, count in group_stats.items()])
        })
...
def save_roll_call_history(class_name: str, students: List[Dict[str, Any]],
                          draw_session: Dict[str, Any]) -> bool:
    """保存点名历史记录
    ...
    try:
        ...
        # 更新元数据
        history_data["metadata"]["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
